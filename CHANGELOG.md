...
- `npm run typecheck`, `npm run lint`, and `npm run test:coverage`.
- ESLint flat config and Prettier, replacing the homegrown formatting scripts.
- GitHub Actions for CI and npm releases with provenance.
- `hrefAwareComponentNames` is now exported, so consumers and tests can see which components sanitize
  their `href`.
- Test coverage for the sanitizer across every href-aware component, plus a guard test that fails if
  a component renders an `href` without being registered as href-aware.
...
- Both browser bundles are now produced by the same esbuild path, eliminating divergent build output.
