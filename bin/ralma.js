...
Ractive.components['dropdown-trigger'] = Ractive.extend({
  isolated: true,
  template:
    "<div class='dropdown-trigger {{class}}' {{#if id}} id='{{id}}' {{/if}}>{{yield}}</div>",
});
Ractive.components['dropdown-button'] = Ractive.extend({
  isolated: true,
  template:
    "<button class='button {{class}}' {{#if id}} id='{{id}}' {{/if}} aria-haspopup='true' aria-controls='dropdown-menu'><span>{{yield}}</span><span class='icon is-small'><i class='fas fa-angle-down' aria-hidden='true'></i></span></button>",
});
Ractive.components['dropdown-item'] = Ractive.extend({
  isolated: true,
  template:
    "<a {{#if href}} href='{{href}}' {{/if}} class='dropdown-item {{class}} {{#active}}is-active{{/}}' {{#if id}} id='{{id}}' {{/if}} {{#if onclick}} onclick='{{onclick}}' {{/if}}>{{yield}}</a>",
});
Ractive.components['dropdown-menu'] = Ractive.extend({
  isolated: true,
  template:
    "<div class='dropdown-menu {{class}}' id='dropdown-menu {{id}}' role='menu {{role}}'>{{yield}}</div>",
});
Ractive.components['dropdown-content'] = Ractive.extend({
  isolated: true,
  template:
    "<div class='dropdown-content {{class}}' {{#if id}} id='{{id}}' {{/if}}>{{yield}}</div>",
});
Ractive.components['dropdown-divider'] = Ractive.extend({
  isolated: true,
  template: "<hr class='dropdown-divider {{class}}' {{#if id}} id='{{id}}' {{/if}}>",
});

/* Content */
Ractive.components['content'] = Ractive.extend({
  isolated: true,
  template:
...
/* Columns */
Ractive.components['columns'] = Ractive.extend({
  isolated: true,
  template:
